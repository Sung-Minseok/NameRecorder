...
export default function FingerBoardScreen(props) {
      ...
      <View style={styles.modalContainer}>
        ...
        >
          <View style={{alignItems: 'flex-start', justifyContent: 'flex-start'}}>
            <Text style={{fontSize: 20, alignItems: 'flex-start', justifyContent: 'center', alignSelf: 'flex-start'}}>{'상담 고객명 : '+boardItem.title}</Text>
            <Text style={{fontSize: 20, alignItems: 'flex-start', justifyContent: 'center'}}>{'문의 사항 : '+boardItem.contents}</Text>
          </View>
          <View style={{flexDirection: 'row',width: DEVICE_WIDTH, justifyContent: 'space-around', alignItems:'center', marginTop:20}}>
            <Text style={{fontSize: 18}}>왼손</Text>
            <Text style={{fontSize: 18}}>오른손</Text>
          </View>
          <FlatList
            data={photoList}
            extraData={photoList}
  ...
  const Item2 = ({ url }) => (
    <View>
      <Image 
            style={{width:DEVICE_WIDTH*0.45, height:DEVICE_WIDTH*0.6, margin: 5}}
            source={{uri: url}}/>
      {/* <Text>{'sss'}</Text> */}
    </View>
  ...
  const getInfo = async () => {
    ...
    const q = query(boardRef, where("uid", "==", uid));
  ...
  const getPhoto = async () => {
    ...
    const listRef = storage.ref(s, uid+'/'+boardItem.title)
    const folderList = storage.listAll(listRef)
    const list = await Promise.all(
      (await folderList).items.map((e)=>{
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Text style={{ fontSize: 18, fontWeight: "bold" }}>상담 이름</Text>
          </View>
          <View
            style={{
              borderRightWidth: 2,
